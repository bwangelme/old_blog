...
例如我利用 strace 查看 feishu app 的系统调用，看到以下的信息
...
我想知道 fd 4 代表了什么，执行 `sudo lsof -d 4 -a -p 8261`，可以看到它是我的电脑和 `220.181.131.241` 建立的一个 TCP 连接。
...
有时候我们启动程序的时候，发现端口被占用了，可以用 `lsof -i:xxx` 来查看被哪个进程占用了，例如:
